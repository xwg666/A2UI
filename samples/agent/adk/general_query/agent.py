...
class GeneralQueryAgent:
    ...
    async def stream(self, query: str, session_id: str) -> AsyncIterable[dict[str, Any]]:
        ...
        #     return
        # if '单选' in query:
        #     logger.info("=== 检测到单选场景，直接返回固定 A2UI ===")
        #     a2ui_json = json.dumps(single_result, ensure_ascii=False)
        #     yield {"is_task_complete": True, "content": f"请在下面的列表中选择您需要的选项---a2ui_JSON---{a2ui_json}"}
        #     return
...
请生成相应的 A2UI 界面。"""
        ...
        while attempt <= max_retries:
            ...
            logger.info(f"  content: {final_content}" )
            ...
            if attempt <= max_retries:
                ...
                current_query = f"""上次响应格式错误: {error_msg}，请重新生成。{current_query}"""
            else:
...
请重新生成。{current_query}"""
    ...
    def _validate_and_clean_a2ui_json(self, content: str) -> tuple[bool, str, str]:
        ...
        try:
            ...
            # 验证失败，返回 False 以触发重试
            return False, content, f"JSON 验证失败: {e}"
    ...
    def _try_repair_json(self, json_str: str) -> list | None:
        ...
        # 3. 移除文本内容，只保留 JSON 部分
        if "---a2ui_JSON---" in repaired:
            repaired = repaired.split("---a2ui_JSON---")[-1].strip()
        ...
        # 6. 尝试修复常见错误：缺少逗号、引号等
        repaired = re.sub(r'([a-zA-Z0-9"\]])\s*([a-zA-Z"\[{])', r'\1,\2', repaired)
        ...
        for i in range(len(repaired), 0, -1):
            try:
                parsed = json.loads(repaired[:i])
                logger.info(f"JSON 修复成功，截断 {len(repaired) - i} 个无效字符")
                return parsed
            except json.JSONDecodeError:
                continue
    ...
    def _remove_forbidden_props(self, data):
        ...
        # 不允许的组件类型
        forbidden_components = {'VideoPlayer', 'AudioPlayer', 'WebFrame'}
