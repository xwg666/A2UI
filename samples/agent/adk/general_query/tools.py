...
def query_data(data: list, query: str) -> list:
    ...
    # 如果没有找到具体匹配，判断是否是泛查询
    if not results:
        is_person_query = any(kw in query_lower for kw in ["人", "员工", "姓名", "联系", "电话", "职位", "性别", "年龄"])
        is_food_query = any(kw in query_lower for kw in ["菜", "菜品", "食物", "吃", "价格", "菜单", "什么菜", "有什么菜"])
        is_it_query = any(kw in query_lower for kw in ["电脑", "IT", "商品", "产品", "物品", "东西", "卖", "购物", "买"])
        
        logger.info(f"  - 泛查询判断: person={is_person_query}, food={is_food_query}, IT={is_it_query}")
