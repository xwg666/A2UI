...
class PeopleFinderAgent:
    ...
    def get_agent_card(self) -> AgentCard:
        capabilities = AgentCapabilities(
            streaming=True,
            extensions=[self._schema_manager.get_agent_extension()] if self.use_ui else [],
        )
        skill = AgentSkill(
            id="find_people",
            name="Find People",
            description="查询公司员工信息",
            tags=["people", "employee", "directory"],
            examples=["查询张三", "技术部有哪些人"],
        )
        return AgentCard(
    ...
    async def stream(self, query: str, session_id: str) -> AsyncIterable[dict[str, Any]]:
        ...
        while attempt <= max_retries:
            attempt += 1
            logger.info(f"--- PeopleFinderAgent: Attempt {attempt}/{max_retries + 1} ---")
            ...
            if self.use_ui:
                try:
                    ...
                    json_cleaned = json_str.strip().lstrip("```json").rstrip("```").strip()
